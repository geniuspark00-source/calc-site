...
export default function CalculatorUI() {
  const [distance, setDistance] = useState(10); // 편도 거리(km)
  const [speed, setSpeed] = useState(40); // 평균 속도(km/h)
  const [fuelCost, setFuelCost] = useState(1800); // 리터당 가격
  const [efficiency, setEfficiency] = useState(12); // 연비(km/L)
  const [altCost, setAltCost] = useState(0); // 대중교통 비용(선택)

  // === 계산 ===
  const commuteTimeOneWay = distance / speed; // 시간(h)
  const commuteTimeDaily = commuteTimeOneWay * 2;
  const commuteTimeMonthly = commuteTimeDaily * 22; // 월 22일 기준
  const commuteTimeYearly = commuteTimeDaily * 264; // 연 264일
  ...
  return (
    <div className="space-y-6">
      ...
      <ResultBox
        title="출퇴근 시간"
        results={[
          { label: "하루", value: `${(commuteTimeDaily * 60).toFixed(0)} 분` },
          {
            label: "월(22일)",
