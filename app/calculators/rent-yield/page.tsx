...
export const metadata = {
  ...
  description: "매입가, 보증금, 월세, 대출금, 이자율 등을 입력하면 자동으로 임대 수익률이 계산됩니다.",
};

export default function RentYieldCalculator() {
  const [price, setPrice] = useState(0);          // 매입가
  const [deposit, setDeposit] = useState(0);      // 보증금
  const [monthly, setMonthly] = useState(0);      // 월세
  const [loan, setLoan] = useState(0);            // 대출금
  const [rate, setRate] = useState(0);            // 대출이자율
  const [manage, setManage] = useState(0);        // 관리비/기타 비용
  ...
  const yieldPercent = netInvestment > 0 ? (annualIncome / netInvestment) * 100 : 0;
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold mb-4 text-blue-700">임대 수익률 계산기</h1>
      ...
      <div className="mt-6 bg-blue-50 p-4 rounded-lg border">
        ...
        <p>월 대출이자: <strong>{Math.round(monthlyLoanInterest).toLocaleString()} 원</strong></p>
        <p>연 순수익: <strong>{Math.round(annualIncome).toLocaleString()} 원</strong></p>
        <p>
          임대 수익률:
