...
export default function UnemploymentBenefitCalculatorUI() {
  ...
  while (remaining > 0) {
    const days = Math.min(30, remaining);
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      ...
      <p className="text-gray-600 mb-6 leading-relaxed">
        실업급여는 평균임금, 근속기간, 연령을 기반으로 지급일수와 지급액이 결정되며,
        <br />실수령액은 3.3% 세금 공제가 적용됩니다.
      </p>

      {/* 입력 영역 */}
      <div className="bg-white p-4 rounded-lg border shadow-sm space-y-4 mb-6">

        <Input label="월 평균임금 (원)" value={avgWage} onChange={setAvgWage} />
        ...
        <Input label="나이" value={age} onChange={setAge} />
      </div>

      {/* 결과 영역 */}
      <div className="bg-white p-4 rounded-lg border shadow-sm space-y-4">

        <h2 className="text-lg font-bold text-blue-700">실업급여 예상 결과</h2>

        <p>
          1일 실업급여 (세전):{" "}
          <strong>{format(dailyBenefit)} 원</strong>
        </p>

        <p>
          1일 실수령액 (세후):{" "}
          <strong className="text-green-700">{format(dailyAfterTax)} 원</strong>
        </p>
        ...
        <p>
          총 수령액 (세전):{" "}
          <strong>{format(totalBeforeTax)} 원</strong>
        </p>
      ...
      <p className="text-[12px] text-gray-500 mt-4 leading-relaxed">
        ※ 실제 수령액은 고용노동부 정산 규정 및 수급 자격에 따라 달라질 수 있습니다.
      </p>
    </div>
  );
}
